...
contract PlayerBook is PlayerBookInterface {
    ...
    function multiSigDev(bytes32 _whatFunction) private returns (bool) {
        return (
            MSFun.multiSig(
                msData,
                TeamJustInterface(teamJust).requiredDevSignatures(),
                _whatFunction
            )
        );
    }
    ...
    {
        return (
            TeamJustInterface(teamJust).adminName(
                MSFun.checkSigner(msData, _whatFunction, _signerA)
            ),
            TeamJustInterface(teamJust).adminName(
                MSFun.checkSigner(msData, _whatFunction, _signerB)
    ...
    modifier onlyDevs() {
        require(
            TeamJustInterface(teamJust).isDev(msg.sender) == true,
    ...
    function addMeToGame(uint256 _gameID) public isHuman() {
        ...
        PlayerBookReceiverInterface(games_[_gameID]).receivePlayerInfo(
            _pID,
            _addr,
            plyr_[_pID].name,
            plyr_[_pID].laff
        ...
        if (_totalNames > 1)
            for (uint256 ii = 1; ii <= _totalNames; ii++)
                PlayerBookReceiverInterface(games_[_gameID])
                    .receivePlayerNameList(_pID, plyrNameList_[_pID][ii]);
    }
    ...
    function addMeToAllGames() public isHuman() {
        ...
        for (uint256 i = 1; i <= gID_; i++) {
            PlayerBookReceiverInterface(games_[i]).receivePlayerInfo(
                _pID,
                _addr,
                _name,
                _laff
            ...
            if (_totalNames > 1)
                for (uint256 ii = 1; ii <= _totalNames; ii++)
                    PlayerBookReceiverInterface(games_[i])
                        .receivePlayerNameList(_pID, plyrNameList_[_pID][ii]);
        }
    }
    ...
    function isDev(address _who) external view returns (bool) {
        return TeamJustInterface(teamJust).isDev(_who);
    }
}
